...
```bash
cd /tmp/testdir/
git clone https://github.com/hashk1/simplechat.git
cd simplechat

```

### 2-1. エンドポイントの設定
FastAPIで作成したエンドポイントを設定する
- `.env_template`の`ENDPOINT_URL`にエンドポイントURLを入力する
```bash
vi lambda/.env_template
```

ファイルの中身
```bash
# 注意：クォーテーション「'」やダブルクォーテーション「"」で囲まないこと
ENDPOINT_URL=ここにエンドポイントURLを入力する
```

- `.env_template`を`.env`に変更する
```bash
mv lambda/.env_template lambda/.env`
```
